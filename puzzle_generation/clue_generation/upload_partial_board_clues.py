#!/usr/bin/env python3
"""
Upload previously generated partial-board clues from a JSONL file to the API.

This script reads NDJSON records from `partial_board_clues.jsonl` (or a provided
path) and uploads them via the admin bulk upload endpoint.

Usage:
  python upload_partial_board_clues.py <local|dev|prod> [PATH_TO_JSONL]
...
DEFAULT_FILE = os.path.join(os.path.dirname(__file__), "partial_board_clues.jsonl")
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Upload partial-board clue NDJSON to the database via API")
    parser.add_argument("env", choices=["local", "dev", "prod"], help="Target environment")
    parser.add_argument("path", nargs="?", default=DEFAULT_FILE, help="Path to JSONL file (default: partial_board_clues.jsonl)")
